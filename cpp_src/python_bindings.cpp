...
template <int DIM> void init(py::module &m) {
    ...
    py::class_<LinSolver<DIM>>(m, (std::string("LinSolver")+std::to_string(DIM)+std::string("D")).c_str())
        ...
        .def("solveGrad", &LinSolver<DIM>::solveGrad)
        .def("solveCurrent", &LinSolver<DIM>::solveCurrent)
        .def("solveCurrentGrad", &LinSolver<DIM>::solveCurrentGrad)
...
PYBIND11_MODULE(network_solver, m) {
    ...
    py::class_<LinSolverResult>(m, "LinSolverResult")
        ...
        .def_readonly("disp_grad", &LinSolverResult::disp_grad)
        .def_readonly("current", &LinSolverResult::current)
        .def_readonly("current_grad", &LinSolverResult::current_grad)
